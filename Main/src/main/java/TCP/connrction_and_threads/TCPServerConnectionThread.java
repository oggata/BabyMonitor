...
public class TCPServerConnectionThread extends Thread{
    ...
    public TCPServerConnectionThread(String ip, int serverPort){
        if (DEBUG) Log.i(TAG, "Starting Connection Thread As Client");
        serverIp = ip;
    ...
    public TCPServerConnectionThread(int serverPort){
        if (DEBUG) Log.i(TAG, "Starting Connection Thread As Server");
        this.serverPort = serverPort;
    ...
    public void close(){
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    if (DEBUG) Log.i(TAG, "Closing the server");
    ...
    private void accept(){
        ...
        try {
            ...
            if (DEBUG) Log.i(TAG, "Waiting for client...");
            ...
            if (socket.isConnected())
            {
                ...
                {
                    if (DEBUG) Log.i(TAG, "Server was closed while accepting");
