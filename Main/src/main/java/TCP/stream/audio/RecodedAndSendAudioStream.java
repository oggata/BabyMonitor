...
public class RecodedAndSendAudioStream extends Thread {
    ...
    public RecodedAndSendAudioStream(OutputStream outputStream){
        ...
        if (DEBUG) Log.d(TAG, "RecodedAndSendAudioStream Created.");
        output = outputStream;
    ...
    public RecodedAndSendAudioStream(OutputStream outputStream, int sampleRate){
        ...
        if (DEBUG) Log.d(TAG, "RecodedAndSendAudioStream Created. Sample Rate: " + sampleRate);
        output = outputStream;
...
//                    if (DEBUG) Log.d(TAG, "Write");
                    ...
                    {
                        if (DEBUG) Log.e(TAG, "No record fail listener");
                        close();
                    }
                }
...
/*Check for timeout in recording if other side stop            Log.d(TAG, "IsRecording!");
            ...
            if (stop)
            {
                if (DEBUG) Log.i(TAG, "Stoping recored!");
                if (audioRecord != null)
                    audioRecord.stop();
