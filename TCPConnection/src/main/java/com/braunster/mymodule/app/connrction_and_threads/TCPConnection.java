...
import com.braunster.mymodule.app.objects.Session;
...
public class TCPConnection {
    ...
    public static final String CONTROLLER_ACTION = "connection.controller.action";
    public static final String CONTROLLER_ACTION_ON = "connection.controller.action.on";
    public static final String CONTROLLER_ACTION_OFF = "connection.controller.action.off";
            ...
            else if (action.equals(ACTION_TOGGLE_CONTROLLER))
            {
                if (intent.getExtras() != null)
                {
                    if (isConnected())
                    {
                        ...
                        if(flags.containsKey(FLAG_READ_XML))
                            return;

                        boolean toSetOn = intent.getExtras().getBoolean(CONTROLLER_ACTION, false);
                        if (DEBUG) Log.d(TAG, "SetToOn: " + String.valueOf(toSetOn));

                        switch (intent.getExtras().getInt(CONTROLLER))
                        {
                            case CONTROLLER_SOUND_RECORDER:
                                if (toSetOn) {
                                    if (DEBUG) Log.d(TAG, "Action Record");
                                    getRecordController().record();
                                }

                                else
                                {
                                    if (DEBUG) Log.d(TAG, "Action Stop Record");
                                    getRecordController().stop();
                                }
                                break;
                            ...
                            case CONTROLLER_SOUND_PLAYER:
                                if (toSetOn)
                                {
                                    if (DEBUG) Log.d(TAG, "Action Play");
                                    getAudioController().play();
                                }
                                else
                                {
                                    if (DEBUG) Log.d(TAG, "Action Stop Audio");
