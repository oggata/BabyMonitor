...
import TCP.stream.video.VideoRecorder;
import TCP.archive.XMLParser;
import TCP.archive.Command;
...
public class TCPCommThread extends Thread {
    ...
    public TCPCommThread(Socket socket) {
        ...
        if (DEBUG) Log.i(TAG, "TCP Communication server is created.");
    ...
    public void run() {
        ...
        while (!isInterrupted())
        {
            if (isStreamOn)
            {
                ...
                {
                    if (DEBUG) Log.d(TAG, "Opening Stream");
            ...
            if (close){
                if (DEBUG) Log.i(TAG, "Closing The Comm Thread.");
                    ...
                    && ( System.currentTimeMillis() - lastCommTime > TCPConnection.TIME_BETWEEN_CHECKS ))
            {
                ...
                if (recodedAndSendAudioStream != null && recodedAndSendAudioStream.isRecording())
                    if (DEBUG) Log.d(TAG, "check while recording ");
    ...
    private boolean playLiveAudio(){
        ...
        if (isStreamsInitialized() && !readXml)
        {
            ...
            if(!loadAndPlayStream.isPlaying())
            {
                ...
                loadAndPlayStream.setAudioStreamForcedStoppedListener(new AudioStreamController.AudioStreamForcedStoppedListener() {
                    @Override
                    public void stopped() {
        ...
        else if (DEBUG) Log.v(TAG, "Stream not initialized or on xml mode");
...
//                    if (DEBUG) Log.d(TAG, " TCP incoming data: " + line);
                ...
                if (line.length() > 1)
                {
                    ...
                    if ( line.length() >= 3 && line.charAt( line.length() -1 ) == Command.END )
                    {
                        ...
                        if (DEBUG) Log.d(TAG, " Save Command: " + command);
    ...
    private void readXml(){
        try {
            if (DEBUG) if (DEBUG) Log.i(TAG, "readXml");
...
//            e.printStackTrace();
            if (DEBUG) Log.e(TAG, "Sleep exception");
        } catch (Exception e) {
//            e.printStackTrace();
            if (DEBUG) Log.e(TAG, "Create XML exception");
        }
    }
